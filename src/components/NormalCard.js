...
const NormalCard = React.forwardRef(
  (
    ...
    const [aspectRatio, setAspectRatio] = useState(0.714) // Standard card aspect ratio (2.5 / 3.5)
    const [isInteractMode, setIsInteractMode] = useState(
      startInteractive || isFlipped
    )
    ...
    useEffect(() => {
      if (applyBoxShadow) {
        switch (rarity) {
          case 'ace spec rare':
            setBoxShadow(
              '0 0 1px -1px #F700C1, 0 0 3px 1px #F700C1, 0 0 12px 2px #F700C1, 0px 10px 20px -5px black, 0 0 20px -30px #F700C1, 0 0 50px -20px #F700C1'
            )
            break
          case 'double rare':
            setBoxShadow(
          ...
          case 'rare rainbow':
            setBoxShadow(
              '0 0 1px -1px rgb(255, 56, 6), 0 0 3px 1px rgb(0, 110, 255), 0 0 12px 2px rgb(66, 255, 66), 0px 10px 20px -5px rgb(255, 51, 0), 0 0 20px -30px rgb(58, 255, 58), 0 0 50px -20px rgb(255, 80, 80)'
            )
            break
          case 'rare ultra':
            setBoxShadow(
    ...
    const handleMouseLeave = () => {
      if (!isInteractMode || !isFlipped || !isRotated || !outerRef.current)
        return
    ...
    const handleCardClick = (e) => {
      if (isInCardOverlay) {
        // Check if the card is in the overlay
        console.log('clicked')
      }
    ...
    const formatSupertypes = (supertypes) => {
      if (Array.isArray(supertypes)) {
        return supertypes
          .map((type) => type.toLowerCase().replace('pokémon', 'pokemon'))
          .join(',')
      } else if (supertypes) {
      ...
      <>
        ...
        <div
          className={`normal-card-wrapper ${
            isTopCard && isFlipped ? getRarityClass() : ''
          }`}
        ...
        >
          <div
            className={`normal-card-outer ${isRotated ? 'rotated' : ''} ${
              transitionBoxShadow ? 'box-shadow-transition' : ''
            }`}
            style={{
