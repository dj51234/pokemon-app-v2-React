  ...
  return (
    <div className={`overlay ${overlayVisible ? 'visible' : 'hidden'}`}>
      ...
      <div className="overlay-content">
        <div
          ref={cardStackRef}
          className="overlay-card-stack"
          style={{ aspectRatio }}
        >
          {isLoading && (
            <div>
              <img src={loadingGif} className="overlay-loading" alt="Loading" />
            </div>
          )}
          {cardStack.map((card, index) => (
            <div
              ...
              className={`overlay-card ${
                card.flipped ? 'overlay-flipped' : ''
              } ${movingCard === index ? 'overlay-card-moving-to-back' : ''}`}
            ...
            >
              <NormalCard
                ...
                boxShadow={
                  index === 1 && !!nextTopCardRarity
                    ? getBoxShadowForRarity(card.rarity)
                    : ''
                }
              />
        ...
        <div className={`overlay-buttons ${lastCardFlipped ? 'visible' : ''}`}>
          <button
            className="gradient-btn btn-primary--pulse btn-open-again"
